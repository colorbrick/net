...
public class SwingChatServer extends JFrame implements ActionListener{
	...
	/* ArrayList 를 사용하지 않고 Vector 를 사용하는 것은 동기화 지원 때문이다.
	 * 소켓 프로그램은 동기화 지원이 필요하다. */
	...
	private void serverStart() {
		...
		try {
			...
			txtList.setFont(new Font("굴림체",Font.BOLD,12)); // 12는 크기
			while (true) {
...
class ChatHandle extends Thread{
	...
	public void run() {
		...
		try{
			...
			// 서버에서 메시지 남기는 기능을 하기 위해.. setMsg() 선언하여 자동생성 시도
			server.setMsg("[ " + nickName +" ] 님이 입장 하셨습니다."); // 자동 생성된 메소드로 가기 위한 단축키 CTRL + 마우스클릭
